...
- **Integración (cabecera `X-API-Key` o `Authorization: Bearer`)**: requiere `scope.permissions` con `accounting:export`. Endpoints: `GET /api/external/accounting/preview`, `GET /api/external/accounting-export/:request_id`, `GET /api/external/accounting-export?from=&to=`, `GET /api/external/export/contable?date_from=`. Cada respuesta deja traza en `api_key_logs` y en el log del servidor.
