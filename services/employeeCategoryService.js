 ...
 */
export async function listCategories(orgId, opts = {}) {
  const where = { orgId };
 ...
 */
export async function getCategory(categoryId, orgId) {
 ...
 */
export async function createCategory(orgId, payload) {
  try {
 ...
 * @returns {Promise<Object>}
 */
export async function updateCategory(categoryId, orgId, payload) {
  const existing = await getCategory(categoryId, orgId);
  if (!existing) {
 ...
 * @returns {Promise<Object>}
 */
export async function deactivateCategory(categoryId, orgId) {
  const existing = await getCategory(categoryId, orgId);
  if (!existing) {
    const err = new Error(`Categoría ${categoryId} no encontrada.`);
    err.status = 404;
