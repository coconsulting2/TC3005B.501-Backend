 ...
 * @param {bigint | number | null} orgId
 * @returns {{ orgId: bigint | number | null, daysAfterTrip: number, graceDays: number, blockOnExpiry: boolean, active: boolean }}
 */
function defaultLimit(orgId) {
  return {
    orgId,
...
export async function getOrgTimeLimit(orgId) {
  ...
  if (!row) return defaultLimit(orgId);
  return {
